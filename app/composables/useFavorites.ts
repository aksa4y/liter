...
export function useFavorites() {
  ...
  function toggle(slug: string) {
    ...
    try { localStorage.setItem('litera:favorites', JSON.stringify(favorites.value)); storageMessage.value = '' }
    catch { storageMessage.value = 'В этом браузере избранное сохранится только до закрытия страницы.' }
  }
  onMounted(() => {
    if (ready.value) return
    try {
      const saved: unknown = JSON.parse(localStorage.getItem('litera:favorites') || '[]')
